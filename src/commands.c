...
COMMAND(cmd_add)
{
	...
	if (params[0] && !match_arg(params[0], 'f', "find", 2) && strcmp(params[0], "$") && !str_to_uin(params[0]) && __uin && (!(u = userlist_find(__uin, NULL)) || !u->display)) {
		const char *name = NULL, *s1 = NULL, *s2 = NULL;
...
void command_init()
{
	...
	( "check_conn", "u?", cmd_check_conn, 0,
	  ...
	  "wiêcej ni¿ jednej wiadomo¶ci testuj±cej w czasie sesji po³±czenia z serwerem - czasami, ze wzglêdu na "
	...
	( "window", "w?", cmd_window, 0,
	  ...
	  "  oldest               prze³±cza do najstarszego okna,\n"
	  "                       w którym co¶ siê dzieje (okna,\n"
	  "                       które najd³u¿ej jest aktywne)\n"
